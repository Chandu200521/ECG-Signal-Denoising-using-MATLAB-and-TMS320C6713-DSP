...
% --- Design Notch Filter (FIR + Hamming Window) ---
f1 = 45;
f2 = 65;
fc = (f1 + f2)/2;
BW = f2 - f1;
...
% Apply filter
filtered_ecg = filtfilt(b, 1, noisy_ecg);

% Normalize for plotting
filtered_ecg = filtered_ecg / max(abs(filtered_ecg));

fprintf('Noise removed using FIR Hamming Window Notch Filter!\n');
...
snr_mse = 10*log10(signal_power / mse_value);

fprintf('SNR (Calculated from MSE) = %.4f dB\n', snr_mse);
